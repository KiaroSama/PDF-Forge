...
def operation_unlock_pdf() -> None:
    ...
    def step_source() -> bool:
        nonlocal source, source_pw, had_open_password, restricted, total_pages, doc
        # Re-entering this step (the user backed up from the output prompt):
    ...
    def step_output() -> bool:
        ...
        return out_path is not None  # back -> source

    try:
        if not navigate_steps([step_source, step_output]):
            return
